 ...
 const handleClick = (changingIndex)=>{
  ...
  return (
    <div className="w-full h-screen bg-zinc-300 gap-4 flex items-center justify-center">
      {data.map((item, index)=>
        <Card key={index} index={index} name={item.name} image={item.image} professsion={item.professsion} handleClick={handleClick} />
      )}
    </div>
  )
...
function Card({image, name, professsion, handleClick, freinds}) {
    ...
    return( 
        <div className='w-52 bg-white rounded-md overflow-hidden'>
            ...
            <div className='w-full p-3'>
                ...
                <button onClick={()=>handleClick(index)} className='px-3 py-1 text-xs text-white bg-blue-500 font-semibold rounded'>{freinds === true ? "Freiend" : "Add Freinds"}</button>
            </div>
        </div>
    )
